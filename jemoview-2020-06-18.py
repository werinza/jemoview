...
#
# jeti model overview
version = 'jemoview version; 20200618'
#
...
def alarms(modelData, fileout):
    fileout.write('\n\nAlarme:')
    fileout.write('\nNummer;Sensor;Wert;X <= / >;Schwellwert;Audio;aktiv')
    ind = 0
    for item in modelData['Alarms']['Data']:  # ist liste von dicts
        ...
        if ind == 1:
            sensor = 'Empfänger'
            parm = 'RX-Spannung'
        out = '\n' + str(ind) + ';' + sensor + ';' + parm + ';' + gt + ';' + str(value) + ';' + audio  + ';' + activt
        fileout.write(out)
...
def displayedtelemetry(modelData, fileout):
    ...
    fileout.write('\nNummer;Inhalt;Zoom')
    sysdisp = [
        '?dt00', 'Flugphasen', 'Antenne', '?dt03', 'RX-Spannung', 'Besitzer', '?dt06',
        'Jetibox', 'Trim', 'Tx Akku', 'Flugzeit', '?dt11', '?dt12', 'Bild', '?dt14'
    ]
    ind = 0
...
def logswitch(modelData, fileout):
    ...
    for ii in range(last -1, -1, -1):
        ...
        if enabled != 0 or label != '' or sw1 != '-' or condt != 'Multi' or sw2 != '-':
            empty = False
            last = ii
            break
    if empty:
        fileout.write('\nkeine Schalter')
        return
    fileout.write('\nNummer;Titel;Schalter1;Verknüpfung;Schalter2;aktiv')
    for item in modelData['LogSwitch']['Data']:  # ist liste von dicts
        ...
        condt = cond[item['Log-Type']]
        sw2 = getSwitch(item['Switch2'])
        out = '\nLog' + str(ind + 1) + ';' + label + ';' + sw1 + ';' + condt + ';' + sw2 + ';' + enabled
        fileout.write(out)
...
def vario(modelData, fileout):
    ...
    for ii in range(len(modelData['Vario']['Setting'])):
        ...
        deadzpos = str(setDecimal(dec, int(modelData['Vario']['Setting'][ii]['DeadZPos'])))
        deadzneg = str(setDecimal(dec, int(modelData['Vario']['Setting'][ii]['DeadZNeg'])))
        minw = str(setDecimal(dec, int(modelData['Vario']['Setting'][ii]['Min'])))
        center = str(setDecimal(dec, int(modelData['Vario']['Setting'][ii]['Center'])))
        maxw = str(setDecimal(dec, int(modelData['Vario']['Setting'][ii]['Max'])))
...
def doIt(modelData, fileout):
    ...
    lua(modelData, fileout)             #schreibt luaid[]
...
if len(sys.argv) == 2:
    ...
    if fileName == '-v' or fileName == '-V' or fileName == '-version' or fileName == '-Version':
        print(version)
        sys.exit(1)
elif len(sys.argv) > 2:
    print('input mehr als 1 parameter')
    sys.exit(1)
else:
